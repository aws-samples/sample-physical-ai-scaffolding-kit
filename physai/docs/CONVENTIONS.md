...
The regression suite lives under `regression/` and is a uv virtual project (`regression/pyproject.toml`, `package = false`). It imports `physai.ssh.Session` and shells out to the `physai` console script; both are provided by `physai` declared as an editable path dep on `../cli` in `[tool.uv.sources]`, so `uv run` pins them to THIS worktree's source rather than a stray editable install elsewhere on the machine. Provision once with `cd regression && uv sync`. Same Python rules as `cli/` (3.10+ type hints, docstrings, ruff), run via `uv run pytest`, with these additions:
...
- Each check function MUST be marked with one of the coverage-layer markers registered in `regression/pyproject.toml` (`[tool.pytest.ini_options]`) (`@pytest.mark.platform` for Layer 1, `@pytest.mark.builtin_example` for Layer 2)
...
- Run unit tests via `cd regression && uv run pytest tests/`. The checks under `physai_regression/checks/` require a live cluster and MUST NOT be run autonomously — see [TIMINGS.md](TIMINGS.md)
