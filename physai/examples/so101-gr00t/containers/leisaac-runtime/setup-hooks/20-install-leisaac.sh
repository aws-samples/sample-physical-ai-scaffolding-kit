...
# IsaacLab 2.3.0 pins dex_retargeting==0.4.6 which brings in old C-extensions
# (pin 2.7.0, nlopt<2.8.0) compiled against numpy 1.x — but IsaacSim 6.0's Kit
# prepends its own numpy 2.3.1 at startup, so importing any of them aborts eval
# with the numpy 1.x/2.x ABI error. Force-upgrade every affected wheel to a
# version built against numpy 2. This also bumps numpy in the venv to 2.x.
pip install --no-cache-dir --upgrade 'pin>=4.0' 'nlopt>=2.11'
pip cache purge || true
