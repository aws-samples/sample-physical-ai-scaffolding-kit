#!/bin/bash
# /app/eval.sh — Container protocol entrypoint for GR00T N1.5 evaluation in LeIsaac.
...
DATA_CONFIG_CLASS=$($GR00T_PYTHON -c "
import ast, sys
...
# ── Start GR00T N1.5 policy server in background ──
T_SERVER_START=$SECONDS
echo "Starting GR00T N1.5 policy server on port $PORT..."
PYTHONPATH="$MODEL_CONFIG_DIR:${PYTHONPATH:-}" \
$GR00T_PYTHON "$GR00T_DIR/scripts/inference_service.py" \
  --server \
